...
/**
 * Territory outline for the strat map viewer. Attack uses the stored attack outline;
 * defense uses a **vertical** (left↔right) mirror of that outline so it matches
 * overlays and pins from `stratMapDisplayData` — not the raw `path_def` Y-mirror.
 */
